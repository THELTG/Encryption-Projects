...
# By Koh Kai En 2022
# 
# A simple 16-byte hashing algorithm using
# a customised sbox and pbox for hashing
# 
# Digest size : 16 bytes
# Output size : 16 bytes (in hexadecimal)
# 
# Known issues: 
#   - padding uses the character "="
#     which causes issues such as when
#     the plaintext contains "="
#     at the end (collision resistance compromised)
#
#   - computing power and RAM usage not
#     taken into consideration in the design
#     of the hash (it may or may not be efficient)
#
#   - collision resistance is not guaranteed 
#     (apart from the padding issue) due to lack
#     of research and testing in this algorithm
#
...
def permute(lst):
    ...
    for i in range(length):
        # for a 16 byte digest, there are 14 rounds of permutations before resetting (resets on 15th round)
        computed = (determinant * (length + i)) % length
        lst[i],lst[computed] = lst[computed],lst[i]
    return lst

def substitute(lst:list,operation=True):
    ...
    for iteration in range(15):
        for i in range(len(blocks)):
            ...
            for j in blocks[i]:
                ...
                ctr += j * 3 # 3 constant to enhance avalanche effect even more
